...
// ══════════════════════════════════════════════════════════════════
//  PLANET EXPLOSION + PHOTO MERGE → "I LOVE YOU" + HEART
// ══════════════════════════════════════════════════════════════════
...
let photoParticleGroups = [];      // original photo point‐cloud references
let photoParticlesTarget = [];     // per-particle target positions
...
function explodePlanet() {
  ...
  // Create burst particles from planet surface
  const count = 1800;
  ...
  explosionParticles.userData.age = 0;
...
// ── 2. Sample heart + text target positions ────────────────────────
function buildLoveTargetPositions() {
  const targets = [];

  // ---- TEXT "I LOVE YOU" via canvas sampling ----
  ...
  tCtx.fillStyle = '#fff';
  tCtx.font = 'bold 80px Arial, sans-serif';
  tCtx.textAlign = 'center';
  tCtx.textBaseline = 'middle';
  tCtx.fillText('I LOVE YOU', tCanvas.width / 2, tCanvas.height / 2);
  const tData = tCtx.getImageData(0, 0, tCanvas.width, tCanvas.height).data;

  // ---- HEART shape via canvas sampling ----
  ...
  hCtx.fillStyle = '#fff';
  hCtx.beginPath();
  const s = 128, cx2 = s / 2, cy2 = s / 2;
  hCtx.moveTo(cx2, cy2 + s * 0.25);
  hCtx.bezierCurveTo(cx2 - s * 0.6, cy2 - s * 0.1, cx2 - s * 0.6, cy2 - s * 0.55, cx2, cy2 - s * 0.25);
  hCtx.bezierCurveTo(cx2 + s * 0.6, cy2 - s * 0.55, cx2 + s * 0.6, cy2 - s * 0.1, cx2, cy2 + s * 0.25);
  hCtx.fill();
  const hData = hCtx.getImageData(0, 0, 128, 128).data;

  // Scale text to world units – match heart height
  const worldH = 14;  // height of text in world units
  const worldW = worldH * (tCanvas.width / tCanvas.height); // aspect

  // Heart size = same height as text, placed to the right
  const heartWorldSize = worldH;

  // Total width: text + gap + heart
  const gap = 4;
  const totalW = worldW + gap + heartWorldSize;
  const startX  = -totalW / 2;
  const textStartX = startX;
  const heartStartX = startX + worldW + gap;

  // Sample text pixels
  const step = 3;
  for (let py = 0; py < tCanvas.height; py += step) {
    for (let px = 0; px < tCanvas.width; px += step) {
      const idx = (py * tCanvas.width + px) * 4;
      if (tData[idx] > 128) {
        targets.push(new THREE.Vector3(
          textStartX + (px / tCanvas.width)  * worldW,
          -(py / tCanvas.height - 0.5) * worldH,
          0
        ));
      }
    }
  ...
  // Sample heart pixels
  const hStep = 3;
  for (let py = 0; py < 128; py += hStep) {
    for (let px = 0; px < 128; px += hStep) {
      const idx = (py * 128 + px) * 4;
      if (hData[idx] > 128) {
        targets.push(new THREE.Vector3(
          heartStartX + (px / 128) * heartWorldSize,
          -(py / 128 - 0.5) * heartWorldSize,
          0
        ));
      }
    }
...
// ── 3. Collect all photo particles into merge pool ─────────────────
function collectPhotoParticles() {
  const pool = [];
  scene.traverse(obj => {
    if (obj.isPoints && obj.userData.materialFar) {
      const posAttr = obj.geometry.getAttribute('position');
      for (let i = 0; i < posAttr.count; i++) {
        const wp = new THREE.Vector3(
          posAttr.getX(i) + obj.position.x,
          posAttr.getY(i) + obj.position.y,
          posAttr.getZ(i) + obj.position.z,
        );
        pool.push({ obj, idx: i, worldPos: wp });
      }
    }
  });
  return pool;
...
// ── 4. Main love formation mesh (canvas sprite plane) ─────────────
function createLoveFormationMeshes(worldY) {
  // ---- "I LOVE YOU" text mesh ----
  const tCanvas = document.createElement('canvas');
  tCanvas.width  = 1024;
  tCanvas.height = 256;
  const tCtx = tCanvas.getContext('2d');
  ...
  const tMat = new THREE.MeshBasicMaterial({ map: tTex, transparent: true, opacity: 0, depthWrite: false, side: THREE.DoubleSide });
  const tGeo = new THREE.PlaneGeometry(40, 10);
  loveTextMesh = new THREE.Mesh(tGeo, tMat);
  loveTextMesh.position.set(-10, worldY, 0);
  ...
  hCtx.shadowColor = '#ff69b4';
  hCtx.shadowBlur  = 30;
  hCtx.fillStyle   = '#ff1493';
  const sz = 256, hcx = sz / 2, hcy = sz / 2;
  hCtx.beginPath();
  hCtx.moveTo(hcx, hcy + sz * 0.25);
  hCtx.bezierCurveTo(hcx - sz * 0.6, hcy - sz * 0.1, hcx - sz * 0.6, hcy - sz * 0.55, hcx, hcy - sz * 0.25);
  hCtx.bezierCurveTo(hcx + sz * 0.6, hcy - sz * 0.55, hcx + sz * 0.6, hcy - sz * 0.1, hcx, hcy + sz * 0.25);
  hCtx.fill();

  const hTex = new THREE.CanvasTexture(hCanvas);
  const hMat = new THREE.MeshBasicMaterial({ map: hTex, transparent: true, opacity: 0, depthWrite: false, side: THREE.DoubleSide });
  const hGeo = new THREE.PlaneGeometry(10, 10);   // same height as text (10 units)
  loveHeartMesh = new THREE.Mesh(hGeo, hMat);
  loveHeartMesh.position.set(25, worldY, 0);       // placed right of text
  scene.add(loveHeartMesh);
}

// ── 5. Animate explosion & merge ──────────────────────────────────
let _explodeAge = 0;
let _mergePool  = null;
let _mergeTargets = null;
let _mergeStartPositions = null;

function animateLoveFormation(delta) {
  if (!loveFormationActive) return;

  // Phase A – explosion burst (0 … 90 frames)
  if (explosionParticles) {
    ...
    if (_explodeAge > 90) {
      ...
      // Start merge phase
      mergePhase = true;
      _mergePool   = collectPhotoParticles();
      _mergeTargets = buildLoveTargetPositions();
      createLoveFormationMeshes(0);

      // Build start positions (world positions of photo particles)
      // We'll animate a subset matching target count
      const useCount = Math.min(_mergePool.length, _mergeTargets.length);
      _mergeStartPositions = [];
      for (let i = 0; i < useCount; i++) {
        _mergeStartPositions.push(_mergePool[i].worldPos.clone());
      }
    }
  }

  // Phase B – merge photo particles toward love shape
  if (mergePhase && _mergePool && _mergeTargets) {
    mergeProgress = Math.min(mergeProgress + 0.008, 1);
    const eased = mergeProgress < 0.5
      ? 2 * mergeProgress * mergeProgress
      : -1 + (4 - 2 * mergeProgress) * mergeProgress;

    const useCount = Math.min(_mergePool.length, _mergeTargets.length);

    // Move each pool particle toward target
    for (let i = 0; i < useCount; i++) {
      const entry = _mergePool[i];
      const start  = _mergeStartPositions[i];
      const target = _mergeTargets[i];

      const nx = start.x + (target.x - start.x) * eased - entry.obj.position.x;
      const ny = start.y + (target.y - start.y) * eased - entry.obj.position.y;
      const nz = start.z + (target.z - start.z) * eased - entry.obj.position.z;

      const posAttr = entry.obj.geometry.getAttribute('position');
      posAttr.setXYZ(entry.idx, nx, ny, nz);
      posAttr.needsUpdate = true;
    }

    // Hide particles not part of formation
    if (mergeProgress > 0.5) {
      scene.traverse(obj => {
        if (obj.isPoints && obj !== explosionParticles && obj.userData.materialFar) {
          // fade toward invisible for non-merged
        }
      });
    }

    // Fade IN love meshes as merge completes
    if (mergeProgress > 0.7 && loveTextMesh && loveHeartMesh) {
      const t = (mergeProgress - 0.7) / 0.3;
      loveTextMesh.material.opacity  = t;
      loveHeartMesh.material.opacity = t;

      // Gentle float & face camera
      loveTextMesh.lookAt(camera.position);
      loveHeartMesh.lookAt(camera.position);
      const bobY = Math.sin(performance.now() * 0.001) * 0.5;
      loveTextMesh.position.y  = bobY;
      loveHeartMesh.position.y = bobY;

      // Pulse heart scale
      const pulse = 1 + 0.06 * Math.sin(performance.now() * 0.003);
      loveHeartMesh.scale.set(pulse, pulse, 1);
    }

    if (mergeProgress >= 1) {
      // Formation complete – keep floating, stop merge logic
      mergePhase = false;
      _mergePool  = null;
    }
  }

  // Continuous float after formation done
  if (!mergePhase && loveTextMesh && loveHeartMesh && loveTextMesh.material.opacity >= 1) {
    const t2 = performance.now() * 0.001;
    const bobY = Math.sin(t2 * 0.8) * 0.6;
    loveTextMesh.position.y  = bobY;
    loveHeartMesh.position.y = bobY;
    loveTextMesh.lookAt(camera.position);
    loveHeartMesh.lookAt(camera.position);
    const pulse = 1 + 0.07 * Math.sin(t2 * 2.5);
    loveHeartMesh.scale.set(pulse, pulse, 1);
  }
}

// ── 6. Entry point called after camera anim finishes ──────────────
function triggerPlanetExplosionAndLoveFormation() {
  loveFormationActive = true;
  explodePlanet();
}

// ── Hook into main animate() loop ─────────────────────────────────
const _origAnimate_loveHook = animate;   // will be patched below
