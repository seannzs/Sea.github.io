...
const ringTexts = [
  'For My Girlfriend ',//untuk deskripsi planet layer 1
  "I love you",//untuk deskripsi planet layer 2
  "♡Happy Anniversary♡",//untuk deskripsi planet layer 3
  "18/05/2026",//untuk deskripsi planet layer 4
...
function animate() {
  ...
  // Love formation (planet explosion → photo merge)
  animateLoveFormation();
  ...
  if (planet.visible) planet.lookAt(camera.position);
  // Only run planet anim if still visible
  if (planet.visible) animatePlanetSystem();
  else animatePlanetSystem && window.textRings && animatePlanetSystem();
...
// ══════════════════════════════════════════════════════════════════
//  PLANET EXPLOSION + "I LOVE YOU" → CINEMATIC RETURN → ANNIVERSARY
// ══════════════════════════════════════════════════════════════════

let loveFormationActive = false;
let explosionParticles = null;
let loveTextMesh = null;
let loveHeartMesh = null;
let loveFormationProgress = 0;
let photoParticleGroups = [];
let photoParticlesTarget = [];
let mergePhase = false;
let mergeProgress = 0;

// ── Cinematic state machine ──────────────────────────────────────
// Phases: 'idle' → 'explode' → 'love_show' → 'love_hold' →
//         'love_fade' → 'planet_restore' → 'orbit' → 'zoom_in' →
//         'zoom_out' → 'black' → 'anniversary'
let cinematicPhase = 'idle';
let cinematicTimer = 0;          // frame counter within phase
let orbitAngle = 0;
let orbitRadius = 60;
let orbitHeight = 20;

// overlay div for black-screen & anniversary text
let anniversaryOverlay = null;
function createAnniversaryOverlay() {
  if (anniversaryOverlay) return;
  anniversaryOverlay = document.createElement('div');
  anniversaryOverlay.style.cssText = `
  ...
  // ── Ganti teks ucapan di sini ──
  const title = document.createElement('div');
  title.innerText = '🎉 Happy Anniversary 🎉';
  title.style.cssText = `
    color:#fff;font-size:clamp(22px,6vw,48px);font-weight:bold;
  ...
  sub.innerText = '❤ Semoga cinta kita abadi selamanya ❤';  // ← ubah di sini
  sub.style.cssText = `
    color:#ffb3de;font-size:clamp(14px,3.5vw,26px);
    font-family:Arial,sans-serif;text-align:center;
    text-shadow:0 0 20px #ff69b4;
    opacity:0;transition:opacity 1.5s ease 0.8s;
    padding:0 20px;
  `;

  const sub2 = document.createElement('div');
  sub2.innerText = '— With all my love 💕 —';            // ← ubah di sini
  sub2.style.cssText = `
    color:#e0b3ff;font-size:clamp(12px,3vw,22px);
    font-family:Arial,sans-serif;text-align:center;
    text-shadow:0 0 15px #e0b3ff;margin-top:14px;
    opacity:0;transition:opacity 1.5s ease 1.6s;
  `;

  anniversaryOverlay.appendChild(title);
  anniversaryOverlay.appendChild(sub);
  anniversaryOverlay.appendChild(sub2);
  document.body.appendChild(anniversaryOverlay);
  anniversaryOverlay._title = title;
  anniversaryOverlay._sub   = sub;
  anniversaryOverlay._sub2  = sub2;
}
...
function explodePlanet() {
  ...
  // Hide text rings immediately
  if (window.textRings) {
    window.textRings.forEach(r => { r.visible = false; });
  }

  // Hide ALL photo particle groups
  scene.traverse(obj => {
    if (obj.isPoints && obj.userData.materialFar) obj.visible = false;
  });
  ...
  const colours = [
    new THREE.Color(0xff69b4), new THREE.Color(0xff1493),
    new THREE.Color(0xffd700), new THREE.Color(0xff6ef7),
  ...
  const mat = new THREE.PointsMaterial({
    size:1.2, vertexColors:true, transparent:true, opacity:1.0,
    depthWrite:false, blending:THREE.AdditiveBlending,
  });
  explosionParticles = new THREE.Points(geo, mat);
  explosionParticles.userData.vel = vel;
  scene.add(explosionParticles);
  galaxyMaterial.uniforms.uRippleTime.value = galaxyMaterial.uniforms.uTime.value;
}

// ── 2. Create merged "I LOVE YOU" + heart (combined single plane) ──
function createLoveFormationMeshes() {
  // Combined canvas: "I LOVE YOU ❤"
  ...
  // Draw big heart first (right side)
  const heartX = tCanvas.width * 0.82;
  const heartY = tCanvas.height * 0.5;
  const heartSize = tCanvas.height * 0.55;
  tCtx.save();
  tCtx.shadowColor = '#ff69b4';
  tCtx.shadowBlur  = 50;
  tCtx.fillStyle   = '#ff1493';
  tCtx.beginPath();
  tCtx.moveTo(heartX, heartY + heartSize * 0.3);
  tCtx.bezierCurveTo(heartX - heartSize*0.6, heartY - heartSize*0.1,
                      heartX - heartSize*0.6, heartY - heartSize*0.55,
                      heartX, heartY - heartSize*0.25);
  tCtx.bezierCurveTo(heartX + heartSize*0.6, heartY - heartSize*0.55,
                      heartX + heartSize*0.6, heartY - heartSize*0.1,
                      heartX, heartY + heartSize*0.3);
  tCtx.fill();
  tCtx.restore();

  // Draw "I LOVE YOU" text (left side, 78% width)
  tCtx.shadowColor = '#ff69b4';
  tCtx.shadowBlur  = 40;
  tCtx.fillStyle   = '#ffffff';
  tCtx.font        = 'bold 150px Arial, sans-serif';
  tCtx.textAlign   = 'left';
  tCtx.textBaseline = 'middle';
  tCtx.fillText('I LOVE YOU', 20, tCanvas.height / 2);
  ...
  const tMat = new THREE.MeshBasicMaterial({
    map: tTex, transparent: true, opacity: 0,
    depthWrite: false, side: THREE.DoubleSide
  });
  const tGeo = new THREE.PlaneGeometry(50, 12);
  loveTextMesh = new THREE.Mesh(tGeo, tMat);
  loveTextMesh.position.set(0, 0, 0);
  scene.add(loveTextMesh);

  // No separate heart mesh — it's embedded in the text canvas
  loveHeartMesh = loveTextMesh; // alias so existing refs still work
}

// ── 3. Restore planet & rings for cinematic return ─────────────────
function restorePlanetForCinematic() {
  planet.visible = true;
  centralGlow.visible = true;
  if (window.textRings) {
    window.textRings.forEach(r => { r.visible = true; });
  }
}

// ── 4. Animate explosion & full cinematic sequence ─────────────────
let _explodeAge = 0;
let _mergePool  = null;
let _mergeTargets = null;
let _mergeStartPositions = null;

function animateLoveFormation() {
  if (!loveFormationActive) return;
  cinematicTimer++;

  // ════════ PHASE: explode ════════
  if (cinematicPhase === 'explode') {
    if (explosionParticles) {
      _explodeAge++;
      const posAttr = explosionParticles.geometry.getAttribute('position');
      const vel = explosionParticles.userData.vel;
      for (let i = 0; i < posAttr.count; i++) {
        posAttr.setXYZ(i,
          posAttr.getX(i) + vel[i*3]   * 0.6,
          posAttr.getY(i) + vel[i*3+1] * 0.6,
          posAttr.getZ(i) + vel[i*3+2] * 0.6,
        );
      }
      posAttr.needsUpdate = true;
      const fadeStart = 60;
      if (_explodeAge > fadeStart)
        explosionParticles.material.opacity = Math.max(0, 1-(_explodeAge-fadeStart)/30);
      if (_explodeAge > 90) {
        scene.remove(explosionParticles);
        explosionParticles = null;
        createLoveFormationMeshes();
        cinematicPhase = 'love_show';
        cinematicTimer = 0;
      }
    }
    return;
  ...
  // ════════ PHASE: love_show (fade in text) ════════
  if (cinematicPhase === 'love_show') {
    const prog = Math.min(cinematicTimer / 80, 1);
    if (loveTextMesh) {
      loveTextMesh.material.opacity = prog;
      loveTextMesh.lookAt(camera.position);
      loveTextMesh.position.y = Math.sin(performance.now()*0.001)*0.5;
    }
    if (prog >= 1) { cinematicPhase = 'love_hold'; cinematicTimer = 0; }
    return;
  }

  // ════════ PHASE: love_hold (display for 4 seconds ≈ 240 frames) ════════
  if (cinematicPhase === 'love_hold') {
    if (loveTextMesh) {
      loveTextMesh.lookAt(camera.position);
      const t = performance.now()*0.001;
      loveTextMesh.position.y = Math.sin(t*0.8)*0.6;
      loveTextMesh.material.opacity = 1;
    }
    if (cinematicTimer > 240) { cinematicPhase = 'love_fade'; cinematicTimer = 0; }
    return;
  }

  // ════════ PHASE: love_fade (fade out text, restore planet) ════════
  if (cinematicPhase === 'love_fade') {
    const prog = Math.min(cinematicTimer / 60, 1);
    if (loveTextMesh) loveTextMesh.material.opacity = 1 - prog;
    if (prog >= 1) {
      if (loveTextMesh) { scene.remove(loveTextMesh); loveTextMesh = null; loveHeartMesh = null; }
      restorePlanetForCinematic();
      // set camera for orbit start
      orbitAngle = Math.atan2(camera.position.x, camera.position.z);
      orbitRadius = Math.sqrt(camera.position.x**2 + camera.position.z**2);
      orbitRadius = Math.max(orbitRadius, 50);
      orbitHeight = camera.position.y;
      controls.enabled = false;
      cinematicPhase = 'orbit';
      cinematicTimer = 0;
    }
    return;
  }

  // ════════ PHASE: orbit (camera orbits planet ≈ 300 frames) ════════
  if (cinematicPhase === 'orbit') {
    orbitAngle += 0.012;
    // gradually descend radius and height toward planet
    orbitRadius  = Math.max(orbitRadius  - 0.08, 30);
    orbitHeight  = orbitHeight * 0.995;
    camera.position.set(
    ...
    camera.lookAt(0, 0, 0);
    if (cinematicTimer > 300) { cinematicPhase = 'zoom_in'; cinematicTimer = 0; }
    return;
  }

  // ════════ PHASE: zoom_in (fly straight toward planet) ════════
  if (cinematicPhase === 'zoom_in') {
    const prog = Math.min(cinematicTimer / 120, 1);
    const eased = 1 - Math.pow(1 - prog, 3);
    const startDist = orbitRadius;
    const dist = startDist * (1 - eased) + 2 * eased;  // zoom to distance=2
    camera.position.set(
      Math.sin(orbitAngle) * dist,
      orbitHeight * (1 - eased * 0.8),
      Math.cos(orbitAngle) * dist
    );
    camera.lookAt(0, 0, 0);
    if (prog >= 1) { cinematicPhase = 'zoom_out'; cinematicTimer = 0; }
    return;
  }

  // ════════ PHASE: zoom_out (pull back fast → fade to black) ════════
  if (cinematicPhase === 'zoom_out') {
    const prog = Math.min(cinematicTimer / 90, 1);
    const dist = 2 + prog * 200;
    camera.position.set(
      Math.sin(orbitAngle) * dist,
      prog * 30,
      Math.cos(orbitAngle) * dist
    ...
    camera.lookAt(0, 0, 0);
    // Start fading overlay to black
    if (!anniversaryOverlay) createAnniversaryOverlay();
    anniversaryOverlay.style.opacity = prog.toFixed(3);
    if (prog >= 1) { cinematicPhase = 'black'; cinematicTimer = 0; }
    return;
  }

  // ════════ PHASE: black (hold black, then show anniversary text) ════════
  if (cinematicPhase === 'black') {
    if (cinematicTimer === 30) {
      // Show anniversary text
      anniversaryOverlay.style.opacity = '1';
      anniversaryOverlay._title.style.opacity = '1';
      anniversaryOverlay._sub.style.opacity   = '1';
      anniversaryOverlay._sub2.style.opacity  = '1';
    }
    cinematicPhase = 'anniversary'; // stay here forever
    return;
  }
}

// ── 5. Entry point called after camera anim finishes ──────────────
function triggerPlanetExplosionAndLoveFormation() {
  loveFormationActive = true;
  cinematicPhase = 'explode';
...
function startCameraAnimation() {
  ...
  function animatePath() {
    ...
    if (progress < duration1) {
      ...
      // ── Trigger: planet hancur → foto menyatu → "I Love You" + ❤ ──
      setTimeout(() => triggerPlanetExplosionAndLoveFormation(), 400);
      return;
    }
