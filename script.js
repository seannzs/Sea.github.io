...
// ============================================================
//  Ambil konfigurasi dari config.js (sudah dimuat di index.html)
// ============================================================
const heartImages  = CONFIG.heartImages  || [];
const ringTexts    = CONFIG.ringTexts    || [];
const hintTextStr  = CONFIG.hintText     || '';
const galaxyCfg    = CONFIG.galaxy       || {};
...
// ============================================================
//  Parameter galaksi (dari config)
// ============================================================
const galaxyParameters = {
  count:           galaxyCfg.starCount   || 100000,
  arms:            galaxyCfg.arms        || 6,
  radius:          100,
  spin:            0.5,
  randomness:      0.2,
  randomnessPower: 20,
  insideColor:     new THREE.Color(galaxyCfg.insideColor  || '#d63ed6'),
  outsideColor:    new THREE.Color(galaxyCfg.outsideColor || '#48b8b8'),
};

// ============================================================
//  Hitung distribusi poin per grup foto
// ============================================================
...
// ============================================================
//  Galaksi utama (bintang-bintang latar)
// ============================================================
...
for (let group = 0; group < numGroups; group++) {
  ...
  img.onload = () => {
    ...
    const materialNear = new THREE.PointsMaterial({
      size: 1.8, map: neonTexture, transparent: false,
      alphaTest: 0.2, depthWrite: true, depthTest: true,
      blending: THREE.NormalBlending, vertexColors: true,
    });
    const materialFar = new THREE.PointsMaterial({
      size: 1.8, map: neonTexture, transparent: true,
      alphaTest: 0.2, depthWrite: false,
...
function createShootingStar() {
  ...
  group.add(head);
  group.add(trail);
  group.userData = { curve, progress: 0, speed: 0.001 + Math.random() * 0.001, life: 0, maxLife: 300, head, trail, trailLength, trailPoints };
  scene.add(group);
...
function createTextRings() {
  ...
  for (let i = 0; i < numRings; i++) {
    ...
    const separator  = '   ';
    const segment    = ringTexts[i % ringTexts.length] + separator;
    const segmentWidth = tempCtx.measureText(segment).width;
    ...
    let fullText = '';
    for (let j = 0; j < repeatCount; j++) fullText += segment;
    let finalTextureWidth = segmentWidth * repeatCount;
    if (finalTextureWidth < 1 || !fullText) { fullText = segment; finalTextureWidth = segmentWidth; }
    ...
    const ringMaterial = new THREE.MeshBasicMaterial({
      ...
      alphaTest: 0.01, opacity: 1, depthWrite: false,
    });
    ...
    ringGroup.userData = {
      ringRadius, angleOffset: 0.15 * Math.PI * 0.5,
      speed: 0.002 + 0.00025,
...
function animatePlanetSystem() {
  ...
  window.textRings.forEach((ringGroup, index) => {
    ...
    const tilt  = Math.sin(time * ud.tiltSpeed  + ud.tiltPhase)  * ud.tiltAmplitude;
    const roll  = Math.cos(time * ud.rollSpeed  + ud.rollPhase)  * ud.rollAmplitude;
    const pitch = Math.sin(time * ud.pitchSpeed + ud.pitchPhase) * ud.pitchAmplitude;
    ringGroup.rotation.x = (index / window.textRings.length) * (Math.PI / 1) + tilt;
    ringGroup.rotation.z = roll;
    ringGroup.rotation.y = ud.angleOffset + pitch;
    ringGroup.position.y = Math.sin(time * (ud.tiltSpeed * 0.7) + ud.tiltPhase) * 0.3;
    const pulse   = (Math.sin(time * 1.5 + index) + 1) / 2;
    const textMesh = ringGroup.children[0];
    if (textMesh && textMesh.material) textMesh.material.opacity = 0.7 + pulse * 0.3;
  });
...
function createHintIcon() {
  ...
  const bgMesh = new THREE.Mesh(new THREE.ShapeGeometry(cursorShape), new THREE.MeshBasicMaterial({ color: 0xffffff, side: THREE.DoubleSide }));
  const fgMesh = new THREE.Mesh(new THREE.ShapeGeometry(cursorShape), new THREE.MeshBasicMaterial({ color: 0xffffff, side: THREE.DoubleSide }));
...
function checkOrientation() {
  ...
  document.body.classList.toggle('portrait-mode', isMobilePortrait);
}
...
if (starField && starField.geometry) starField.geometry.setDrawRange(0, Math.floor(originalStarCount * 0.1));
...
function animate() {
  ...
  if (!introStarted) {
    fadeOpacity = 0.1;
    scene.traverse(obj => {
      if (obj.name === 'starfield') return;
      if (obj.userData.isTextRing || (obj.parent && obj.parent.userData && obj.parent.userData.isTextRing)) {
        if (obj.material && obj.material.opacity !== undefined) { obj.material.transparent = false; obj.material.opacity = 1; }
        ...
        if (obj.material && obj.material.opacity !== undefined) { obj.material.transparent = true; obj.material.opacity = 0.1; }
      }
    });
    planet.visible      = true;
    ...
    scene.traverse(obj => {
      if (!(obj.userData.isTextRing || (obj.parent && obj.parent.userData && obj.parent.userData.isTextRing) || obj === planet || obj === centralGlow || obj.type === 'Scene')) {
        ...
        if (obj.material && obj.material.opacity !== undefined) { obj.material.opacity = 1; obj.material.transparent = false; }
      }
      if (obj.material && obj.material.color) obj.material.color.set(0xffffff);
    });
  }
  ...
  for (let i = shootingStars.length - 1; i >= 0; i--) {
    ...
    for (let j = 1; j < star.userData.trailLength; j++) {
      trailPoints[j].copy(star.userData.curve.getPoint(Math.max(0, star.userData.progress - j * 0.01)));
    }
  ...
  // Swap material foto near/far
  scene.traverse(obj => {
    if (obj.isPoints && obj.userData.materialNear && obj.userData.materialFar) {
      const posAttr = obj.geometry.getAttribute('position');
      let isClose = false;
      for (let i = 0; i < posAttr.count; i++) {
        const dist = camera.position.distanceTo(new THREE.Vector3(posAttr.getX(i) + obj.position.x, posAttr.getY(i) + obj.position.y, posAttr.getZ(i) + obj.position.z));
        if (dist < 10) { isClose = true; break; }
      }
      if (isClose) {
        if (obj.material !== obj.userData.materialNear) { obj.material = obj.userData.materialNear; obj.geometry = obj.userData.geometryNear; }
      } else {
        if (obj.material !== obj.userData.materialFar)  { obj.material = obj.userData.materialFar;  obj.geometry = obj.userData.geometryFar; }
      }
    }
  });
