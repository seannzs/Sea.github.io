...
// ══════════════════════════════════════════════════════════════════════════════
//  CINEMATIC CAMERA SYSTEM — smooth spline-based, time-driven, ~50 detik
// ══════════════════════════════════════════════════════════════════════════════
...
// Interpolate position along a Catmull-Rom spline (wraps for looping)
...
function startCameraAnimation() {
  ...
  const TOTAL_DURATION = 50; // detik — total durasi sinematik

  // ── Waypoints posisi kamera (path sinematik) ─────────────────────────────
  // Setiap waypoint: { pos: Vector3, lookAt: Vector3, duration: detik, ease: fn }
  // FOV per fase: terasa dramatis saat close-up, wide saat reveal galaksi
  // fovFrom → fovTo selama fase berlangsung
  const phases = [
    // FASE 1 — Gentle approach: perlahan mendekati planet
    {
      from:     camera.position.clone(),
      to:       new THREE.Vector3(0, 8, 22),
    ...
    // FASE 2 — Cinematic close-up: zoom in sangat dekat, FOV menyempit
    {
      from:     new THREE.Vector3(0, 8, 22),
      to:       new THREE.Vector3(6, 2, 14),
      lookFrom: new THREE.Vector3(0, 0, 0),
    ...
    // FASE 3 — Skim surface: menyapu rendah di permukaan planet
    {
      from:     new THREE.Vector3(6, 2, 14),
      to:       new THREE.Vector3(-14, 1.5, 6),
    ...
    // FASE 4 — Low orbit arc: orbit rendah memutar dari kiri ke belakang
    {
      from:     new THREE.Vector3(-14, 1.5, 6),
      to:       new THREE.Vector3(-18, 8, -12),
      lookFrom: new THREE.Vector3(0, 0, 0),
      lookTo:   new THREE.Vector3(0, 0, 0),
      fovFrom:  40, fovTo: 50,
      duration: 5,
    ...
    // FASE 5 — Rising spiral: naik sambil terus orbit
    {
      from:     new THREE.Vector3(-18, 8, -12),
      to:       new THREE.Vector3(12, 22, -18),
      lookFrom: new THREE.Vector3(0, 0, 0),
      lookTo:   new THREE.Vector3(0, 0, 0),
      fovFrom:  50, fovTo: 60,
      duration: 5,
    ...
    // FASE 6 — Dramatic pull-back: tarik mundur cepat, FOV melebar — REVEAL galaksi
    {
      from:     new THREE.Vector3(12, 22, -18),
      to:       new THREE.Vector3(22, 42, 55),
      lookFrom: new THREE.Vector3(0, 0, 0),
      lookTo:   new THREE.Vector3(0, 0, 0),
      fovFrom:  60, fovTo: 85,
      duration: 5,
    ...
    // FASE 7 — Galaxy sweep: kamera menyapu jauh ke samping galaxy
    {
      from:     new THREE.Vector3(22, 42, 55),
      to:       new THREE.Vector3(85, 12, 25),
      lookFrom: new THREE.Vector3(0, 0, 0),
      lookTo:   new THREE.Vector3(0, 5, 0),
      fovFrom:  85, fovTo: 75,
      duration: 6,
      ease:     smootherstep,
    },
    // FASE 8 — High arc: naik tinggi melihat galaxy dari atas seperti planet bumi dari luar angkasa
    {
      from:     new THREE.Vector3(85, 12, 25),
      to:       new THREE.Vector3(28, 95, 55),
      lookFrom: new THREE.Vector3(0, 5, 0),
      lookTo:   new THREE.Vector3(0, 0, 0),
      fovFrom:  75, fovTo: 70,
      duration: 6,
      ease:     easeInOutQuart,
    },
    // FASE 9 — Dramatic dive swoop: menukik dari tinggi ke posisi rendah
    {
      from:     new THREE.Vector3(28, 95, 55),
      to:       new THREE.Vector3(-22, 48, 112),
      lookFrom: new THREE.Vector3(0, 0, 0),
      lookTo:   new THREE.Vector3(0, 8, 0),
      fovFrom:  70, fovTo: 75,
      duration: 5,
      ease:     easeInOutSine,
    },
    // FASE 10 — Final smooth settle: gerak mulus ke posisi akhir
    {
      from:     new THREE.Vector3(-22, 48, 112),
      to:       new THREE.Vector3(-40, 100, 100),
      lookFrom: new THREE.Vector3(0, 8, 0),
      lookTo:   new THREE.Vector3(0, 0, 0),
      fovFrom:  75, fovTo: 75,
      duration: 4,
      ease:     easeOutQuint,
  ...
  // Hitung cumulative time untuk setiap fase
  let cumulativeTimes = [0];
  for (let i = 0; i < phases.length; i++) {
    cumulativeTimes.push(cumulativeTimes[i] + phases[i].duration);
  }
  const actualTotal = cumulativeTimes[cumulativeTimes.length - 1];
  ...
  function animateCinematic(timestamp) {
    ...
    if (elapsed >= actualTotal) {
      // Selesai — posisi final, reset FOV
    ...
    const phaseElapsed = elapsed - cumulativeTimes[phaseIdx];
    const rawT = Math.min(phaseElapsed / phase.duration, 1);
    const t = phase.ease(rawT);

    // Interpolasi FOV — sinematik zoom in/out
    const targetFOV = phase.fovFrom + (phase.fovTo - phase.fovFrom) * t;
    camera.fov = targetFOV;
    camera.updateProjectionMatrix();

    // Interpolasi posisi
    const newPos = new THREE.Vector3().lerpVectors(phase.from, phase.to, t);

    // Interpolasi look target
    const lookTarget = new THREE.Vector3().lerpVectors(phase.lookFrom, phase.lookTo, t);

    // Camera breathing — micro-movement organik, amplitude mengecil saat close-up
    const distToPlanet = newPos.length();
    const breathScale  = Math.min(distToPlanet / 30, 1) * 0.12;
    const breathFreq   = 0.35;
    const breath = new THREE.Vector3(
      Math.sin(elapsed * breathFreq * 1.3 + 0.5) * breathScale,
      Math.sin(elapsed * breathFreq * 0.8 + 1.2) * breathScale * 0.6,
      Math.sin(elapsed * breathFreq * 1.1 + 2.0) * breathScale * 0.4
    );

    camera.position.copy(newPos).add(breath);

    camera.lookAt(lookTarget);

    raf = requestAnimationFrame(animateCinematic);
  }
