...
function startCameraAnimation() {
  ...
  // ─── PHASE DEFINITIONS ──────────────────────────────────────────────────────
  // Phase 1: Zoom IN ke planet (mendekati dari posisi awal ke dekat planet)
  // Phase 2: Orbit mengelilingi planet (berputar 360° sambil dekat)
  // Phase 3: Pull back + naik (zoom out sambil terangkat)
  // Phase 4: Dramatic swoop — menyapu dari bawah ke posisi akhir
  // ─────────────────────────────────────────────────────────────────────────────

  const startPos   = camera.position.clone();
  const closePos   = new THREE.Vector3(0, 3, 18);   // sangat dekat ke planet
  const orbitRadius = 22;
  const orbitHeight = 5;
  const orbitTurns  = 1.2;                           // 1.2 putaran orbit
  const pullBackPos = new THREE.Vector3(60, 30, 80); // zoom out jauh
  const finalPos    = new THREE.Vector3(-40, 100, 100);

  // Durasi setiap fase dalam "progress" unit (total 0→1)
  const d1 = 0.18;  // zoom in
  const d2 = 0.35;  // orbit
  const d3 = 0.22;  // pull back / zoom out
  const d4 = 0.25;  // dramatic swoop ke final

  let progress = 0;
  const speed  = 0.0018; // kecepatan keseluruhan

  // Simpan sudut orbit awal dari startPos supaya sambungan mulus
  const orbitStartAngle = Math.atan2(startPos.z, startPos.x);
  ...
  function animatePath() {
    ...
    if (p < d1) {
      // ── FASE 1: Cinematic ZOOM IN ──────────────────────────────────────────
      const t = easeInCubic(p / d1);
      newPos.lerpVectors(startPos, closePos, t);

    } else if (p < d1 + d2) {
      // ── FASE 2: ORBIT mengelilingi planet ─────────────────────────────────
      const t = easeInOut((p - d1) / d2);
      const angle = orbitStartAngle + t * Math.PI * 2 * orbitTurns;
      newPos.set(
        Math.cos(angle) * orbitRadius,
        orbitHeight + Math.sin(t * Math.PI * 2) * 4, // sedikit naik-turun
        Math.sin(angle) * orbitRadius
      );
      // Lihat sedikit ke depan searah orbit supaya terasa sinematik
      const lookAhead = angle + 0.3;
      lookTarget.set(
        Math.cos(lookAhead) * 5,
        0,
        Math.sin(lookAhead) * 5
      );

    } else if (p < d1 + d2 + d3) {
      // ── FASE 3: PULL BACK — zoom out cepat ────────────────────────────────
      const t = easeOutCubic((p - d1 - d2) / d3);
      // Mulai dari posisi terakhir orbit
      const orbitEndAngle = orbitStartAngle + Math.PI * 2 * orbitTurns;
      const orbitEndPos = new THREE.Vector3(
        Math.cos(orbitEndAngle) * orbitRadius,
        orbitHeight,
        Math.sin(orbitEndAngle) * orbitRadius
      );
      newPos.lerpVectors(orbitEndPos, pullBackPos, t);

    } else {
      // ── FASE 4: DRAMATIC SWOOP ke posisi akhir ────────────────────────────
      const t = easeInOut((p - d1 - d2 - d3) / d4);
      newPos.lerpVectors(pullBackPos, finalPos, t);
    }
